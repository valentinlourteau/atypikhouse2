<template>
  <div>
   <v-container grid-list>
    <div style="background-image:url(/static/images/slider/atypikhouse.jpg);position:relative;height: 500px;">
    <v-layout wrap 
    align-center
    pa-3
    >
    ...
    offset-md1
    >
    <h1 class="bulle" > Offrez-vous la Magie d'une nuit insolite</h1>
  </v-flex>
  <v-flex xs12
  md7
  offset-ml5
...
<v-flex lg6 xs12 offset-lg3 align-center>
  <div class="subheading text-uppercase pl-2 mt-3">
    <v-text-field prepend-icon="search" v-model="search" :placeholder="getRandomPlaceHolder()" solo></v-text-field>
    <v-btn class="mx-auto black--text" style="display:block;" color="primary" @click="imLucky()" fab >
...
<v-container>
  ...
  <v-dialog v-model="dialog" persistent max-width="580">
    <v-btn slot="activator" color="yellow" text="black">Arrivée-Départ</v-btn>
    <v-card>
      <v-card-title>
  ...
  <v-dialog v-model="dialog1" persistent max-width="580">
    <v-btn slot="activator" color="yellow" text="black">Prix</v-btn>
    <v-card>
...
<v-dialog v-model="dialog2" persistent max-width="580">
  <v-btn slot="activator" color="yellow" text="black">Capacité</v-btn>
  <v-card ml4>
    <!-- Capacité d'accueil -->
    <v-flex xs12>
...
<v-dialog v-model="dialog3" persistent max-width="580">
  <v-btn slot="activator" color="yellow" text="black">Type de logement</v-btn>
  <v-card>
...
data: function() {
  ...
  .separator-transparent-white {
    background-image: url(/static/images/slider/separateur.png);
    margin-top: -15px;
  ...
  .bulle {
    ...
    background-color: rgba(255,255,51,0.85);
    padding: 70px 45px;
    font-size: 22px;
    border-radius: 50%;
    text-align: center;
